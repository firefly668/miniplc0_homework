  ...
  while (true) {
    ...
    switch (current_state) {
      ...
      case UNSIGNED_INTEGER_STATE: {
        ...
        if (!current_char.has_value()){
          int64_t temp = atoi(ss.str().c_str());
          if(temp>2147483647)
            return std::make_pair(std::optional<Token>(),
      ...
      case IDENTIFIER_STATE: {
        ...
        if (!current_char.has_value())
          return checkIDENTIFIER(ss.str(),pos);
...
std::pair<std::optional<Token>,std::optional<CompilationError>> 
Tokenizer::checkIDENTIFIER(std::string s,std::pair<int64_t, int64_t> pos){
    if(s=="begin"){
      return std::make_pair(std::make_optional<Token>(TokenType::BEGIN,s,pos,currentPos()),
                            std::optional<CompilationError>());
    }
    else if(s=="end"){
      return std::make_pair(std::make_optional<Token>(TokenType::END,s,pos,currentPos()),
                            std::optional<CompilationError>());
    }
    else if(s=="var"){
      return std::make_pair(std::make_optional<Token>(TokenType::VAR,s,pos,currentPos()),
                            std::optional<CompilationError>());
    }
    else if(s=="const"){
      return std::make_pair(std::make_optional<Token>(TokenType::CONST,s,pos,currentPos()),
                            std::optional<CompilationError>());
    }
    else if(s=="print"){
      return std::make_pair(std::make_optional<Token>(TokenType::PRINT,s,pos,currentPos()),
                            std::optional<CompilationError>());
    }
    else{
